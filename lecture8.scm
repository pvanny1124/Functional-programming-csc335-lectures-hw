...
(define (multi-insertL lat old new)
  (cond ((null? lat) '())
	((eq? (car lat) old) (cons new (cons (car lat) (multi-insertL (cdr lat) old new)))) 
  (else (cons (car lat) (multi-insertL (cdr lat) old new)))))
  

(define (multi-insertR lat old new)
  (cond ((null? lat) '())
	((eq? (car lat) old) (cons (car lat) (cons new (multi-insertR (cdr lat) old new)))) 
	(else (cons (car lat) (multi-insertR (cdr lat) old new)))))
