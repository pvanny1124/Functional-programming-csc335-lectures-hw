...
;test 1: is invariant strong enough on termination?
    ;when count = n, 
    ;b is returned, in which b is a
    ;where a = (+ a b) or,
    ;the sum of the previous two integers in the fib sequence. 

;test 2: does invariant hold on first call?
    ;for fib(N), the program returns b = n which is the nth fib.
    ;fib(0) effectively returns b = 0 since count and b are equal to 0 on the first call to fib-iter.

;test 3: does it work for (k+1)st call?
    ;for fib(N+1), b = A and a = B + A.
    ;a = B + A is the next integer in the fib sequence
    ;b is the current fib integer.

;since count is incremented at every step, count will eventually be equal to n.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;curr is the nth fib number
;prev is the (n-1)st fib number

;invariant: curr = CURR + PREV

;1st test: is invariant strong enough for termination?
        ;Termination occurs when count = n
        ;count is incremented on every pass and will eventually be equal to n
        ;On termination, curr is the current fib number in the sequence up to fib(N).
        ;which is the result of (+ curr prev) or rather, the last two numbers 
        ;in the fib sequence before curr

;2nd test: does invariant hold on 1st pass?
        ;for fib(0), program returns 0 
        ;for fib(1), the program returns 1
        ;for fib(n > 1), the program calls fib-iter to handle all other cases
        ;on first pass through fib-iter, fib(n = 2),
        ;curr = 1 which is effectively the 2nd fib number

;3rd test: does it work for (k+1)st call?
        ;Taking N to be N = 2,
        ;for fib(N+1), 
        ;curr becomes CURR + PREV (the nth fib number)
        ;prev becomes CURR (the (n-1)st fib number)
        ;the invariant effectively holds, and the design roles are maintained.
...
;test 1: strong on termination?
        ;a is incremented by every call to iter
        ;result-so-far is returned once a = b
  
;test 2: invariant holds on 1st call?
        ;on 1st call, result-so-far = A + (A + 1)
        ;Effectively adding the current integer in the sequence
        ;with the next one.
        ;if b > a, the program skips the cond check
        ;meaning that a <= b still holds.
        ;however, it should be noted that a should be < b.

;test 3: does invariant work on (k+1)st call?
        ;result-so-far = (A + 1) +(A + 1 + 1) = (A + 1) + (A + 2)
        ;(A + 1) being the (A + 1)nth term and (A + 2) being the integer that follows (A + 1).
        ;if b > a, the program skips the cond check and 
        ;if not, 
        ;meaning that a <= b still holds.
...
;Test1: is invariant strong enough on termination?
        ;when a > b, result-so-far = (+ a result-so-far)
        ;since (+ a 1) is passed in as an actual argument for a at each call to iter,
        ;a is incremented to the next number to be added.
        ;A <= i < a is still satisfied because the last call to iter increments a to a + 1
        ;in this case, a (now being a + 1) is > b which returns result-so-far and stops accumulating
        ;integers.
        ;effectively adding the previous number carried in the variable result-so-far.
        ;result-so-far effectively returns the sum upon this termination.

        ;if a > b and a <= b + 1, or a = b + 1, then the program returns 0 as it should.
        ;if a > b in iter and since a and b are integers, it must be the case 
        ;that a = b+1 which is synonomous to a <= b+1

;Test2: passes on 1st call?
        ;on first recursive call to iter, result-so-far is 0 + a
        ;effectively equaling a, or the 1st number in the sequence which is correct.

        ;a <= b + 1 is maintained as long as the cond a > b is not satisfied for 
        ;reasons stated in the test1 explanation

;Test3: A+1?
        ;on the A+1st call, result-so-far is 0 + A + A + 1
        ;a = A + 1
        ;or rather, A + a which is the correct sum of the sequence up until A+1.
...
;Test1: invariant strong enough on termination?
        ;result-so-far on termination is a + result-so-far
        ;result-so-far being the accumulated integers from A <= i < a.
        ;since a must be <= b,
        ;we add a to result-so-far to satisfy a = b

;Test2: passes on 1st call?
        ;on first recursive call to iter,
        ;a = A and
        ;result-so-far is A + 0 which is just A
        ;which is the first integer in the summation and
        ;
